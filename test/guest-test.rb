...
test "guest.reserve(room)",
  ...
  actual   = (
    guest = create_guest
    room  = create_room
    guest.reserve(room_id: room.id, start_date: DateTime.now + 1, end_date: DateTime.now + 2).class
  )
...
test "guest.reservations, with reservations",
  ...
  actual   = (
    guest = create_guest
    room  = create_room
    reservation = guest.reserve(room_id: room.id, start_date: DateTime.now + 1, end_date: DateTime.now + 2)
...
test "guest.rooms, with reservations",
  ...
  actual   = (
    guest = create_guest
    room  = create_room
    guest.reserve(room_id: room.id, start_date: DateTime.now + 1, end_date: DateTime.now + 2)
